...
function loadInfo(){
	if(self.fetch) {
		...
		song_elem.innerHTML = 'Для работы плеера необходим <a href="https://new.vk.com/badbrowser.php" target="_blank" rel="nofollow noopener">современный браузер</a>.';
		if (!container.classList.contains('offline')) {
			container.classList.add('offline');
		}
...
var pp_timer = setInterval(function(){
	loadInfo();
}, 5000);
