<?php
	...
	if(isset($_POST['add_air'])) {
		...
		if($_POST['ellink'] != null) {
			$newar = [$time_start, $time_end, json_decode('"'.$name.'"'), json_decode('"'.$link.'"')];
		} else {
...
<html lang="ru"><head>
	...
	<?php else: ?>
	<meta charset="utf-8">
	<title>Asian Wave Backend</title>
	...
	<form class="add-air" action="?succ" method="post"><fieldset>
		...
		<!--<p><input hidden required type="number" name="secsOffset"></label></p>-->
		<p style="margin-top: 25px;"><input type="submit" value="Записать" name="add_air"><label for="add_air"><i>Внимательно проверьте введённые данные перед отправкой!</i></label></p>
	</fieldset></form>
	...
	<form class="expired-clear" action="?succ" method="post"><fieldset>
		...
		<p><?php// echo '<span class="dev">Просроченных эфиров'; if(($shed_data[0][1] > time()) && ($expr_count != 1)) {echo ' нет.</span>';} else {echo ': ' . $expr_count . '</span>';} ?>
		<p><?php echo '<span class="dev">Просроченных эфиров'; if($shed_data[1][1] > time() && $expr_count = 1) {echo ' нет</span>';} else {echo ': ' . $expr_count . '</span>';} ?>
	...
	<footer>
		<p>Asian Wave Raspisanie Editor v0.1.5</p>
	...
	<script type="text/javascript">
		...
		function lastClVeevod() {
			var l, lTimeS = <?php echo $shed_data[count($shed_data)-2][0]; ?>, lTimeE = <?php echo $shed_data[count($shed_data)-2][1]; ?>, lAirN = '<?php echo $shed_data[count($shed_data)-2][2]; ?>', lVeevodEl = document.createElement('span'), lVeevodElPar = document.querySelector('.lVeevod'), lInputs = document.querySelectorAll('.last-clear input');
			
			lVeevodEl.classList.add('dev');
		...
		function exprCheck() {
			var e, air_exp = <?php if($shed_data[1][1] > time() && $expr_count = 1) {echo 'false';} else {echo 'true';} ?> , exprFInputs = document.querySelectorAll('.expired-clear input');
			if (!air_exp) {
		...
		function toLocalTime() {
			var i, lastTS = <?php echo $shed_data[count($shed_data) - 2][1]; ?>, inputsLT = document.querySelectorAll('input[type*=datetime]')
		...
		function fileUpdate() {
			var shedTS = '<?php echo @filemtime($shed) ?>', shedTSEl = document.querySelector('.shedTS');

			shedTSEl.textContent = moment.unix(shedTS).from();
		}

		/*function UTC_Offset() {
			var secsOffset = moment().utcOffset() / 60;

			document.querySelector('input[name=secsOffset]').setAttribute('value', secsOffset);
			document.querySelector('.secsOffset').textContent = secsOffset + ' часов';
		}*/
