<?php
	...
	$anime_sched = 'anime-sched-v2.json';
	$radio_sched = 'radio-sched.json';
	$noti = 'noti.json';

	$schedAnime = json_decode(file_get_contents($anime_sched), true);
	$schedRadio = json_decode(file_get_contents($radio_sched), true);
	$noti_cnt = json_decode(file_get_contents($noti), true);
	...
	function shedSort($f_what) {
		usort($f_what, function($a, $b) { return $a['s'] - $b['s']; });
		return $f_what;
	}
	...
	function addShedData($f_file, $f_where, $f_what) {
		...
		if ($f_what['title'] != null) { file_put_contents(dirname(__FILE__) . '/' . $f_file, json_encode($f_where, JSON_UNESCAPED_UNICODE)); }
	}
	...
	if (isset($_POST['add_air'])) {
		...
		if ($_POST['ellink'] != null) { $newar['link'] = json_decode('"'.$link.'"'); }
	...
	if (count($schedAnime) > 1) {
		for ($e = 0; $e <= (count($schedAnime) - 2); $e++) {
			if ($schedAnime[$e][1] < time()) { ++$expr_count; }
		}
	} else { $expr_count = 0; }

	if (isset($_POST['expired_clear']) && $expr_count > 0) {
		$temp = [];

		foreach ($schedAnime as &$item) {
			if ($item['e'] > time()) { $temp[count($temp)] = $item; }
		}

		$schedAnime = $temp;
		$schedAnime = shedSort($schedAnime);
...
<html lang="ru"><head>
	...
	<link rel="stylesheet" href="/files/code/css/aw-fonts.min.css">
	<style>
	...
	<form class="last-clear" action="?succ" method="post"><fieldset>
		...
		<p>
			<?php echo '<samp>'; if ($schedAnime[count($schedAnime)-2]['s']) { echo 'Название: <q>' . $schedAnime[count($schedAnime)-2]['title'] . '</q>. Начало ' . date('Y-m-d в H:i', $schedAnime[count($schedAnime)-2]['s']) . '; конец ' . date('Y-m-d в H:i', $schedAnime[count($schedAnime)-2]['e']); } else { echo 'Очистка не требуется'; } echo '.</samp>' ?>
		</p>
		<p>
	...
	<form class="expired-clear" action="?succ" method="post"><fieldset>
		...
		<p>
			<?php echo '<samp>Просроченных эфиров'; if ($schedAnime[0]['s'] > time() && $expr_count = 1) { echo ' нет</samp>'; } else { echo ': ' . $expr_count . '</samp>'; } ?>
		</p>
		<p>
	...
	<script type="text/javascript">
		...
		document.addEventListener('DOMContentLoaded', () => {
			...
			;(function lastClVeevod() {
				let
					lTimeS = <?php echo $schedAnime[count($schedAnime)-2]['s'] ? $schedAnime[count($schedAnime)-2]['s'] : 0; ?>,
					lInputs = $make.qs('.last-clear input', ['a'])

				if (lTimeS == <?php echo $schedAnime[count($schedAnime)-1]['s']; ?>) {
			...
			;(function toLocalTime() {
				let
					lastTS = <?php echo $schedAnime[count($schedAnime) - 2]['e']; ?>,
